...
-- admin@reviewshield.test  / devpass1234   (app_metadata.role = 'admin')
-- klant@reviewshield.test  / devpass1234   (regular user)
--
...
declare
  ...
  v_klant uuid;
begin
  delete from auth.users where email in ('admin@reviewshield.test','klant@reviewshield.test');
  ...
  values ('00000000-0000-0000-0000-000000000000',v_admin,'authenticated','authenticated',
          'admin@reviewshield.test',extensions.crypt('devpass1234',extensions.gen_salt('bf')),now(),
          '{"provider":"email","providers":["email"],"role":"admin"}'::jsonb,
          '{"name":"Dev Admin"}'::jsonb,now(),now(),
          '','','','','','','','');
  insert into auth.identities (id,provider_id,user_id,identity_data,provider,last_sign_in_at,created_at,updated_at)
  values (gen_random_uuid(), v_admin::text, v_admin,
          jsonb_build_object('sub',v_admin::text,'email','admin@reviewshield.test','email_verified',true),
          'email', now(), now(), now());
  ...
  values ('00000000-0000-0000-0000-000000000000',v_klant,'authenticated','authenticated',
          'klant@reviewshield.test',extensions.crypt('devpass1234',extensions.gen_salt('bf')),now(),
          '{"provider":"email","providers":["email"]}'::jsonb,
          '{"name":"Dev Klant"}'::jsonb,now(),now(),
          '','','','','','','','');
  insert into auth.identities (id,provider_id,user_id,identity_data,provider,last_sign_in_at,created_at,updated_at)
  values (gen_random_uuid(), v_klant::text, v_klant,
          jsonb_build_object('sub',v_klant::text,'email','klant@reviewshield.test','email_verified',true),
          'email', now(), now(), now());
end $$;
